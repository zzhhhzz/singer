  ...
  */
      ...
      for (LogMessage msg : logMessages) {
        ...
        if (msg.getLoggingAuditHeaders() != null) {
          ...
          if (msg.isSetChecksum() && singerChecksum != msg.getChecksum()) {
            ...
            // if corrupted messages are configured to be deleted now, skip to the next message
            if (auditConfig.isSkipCorruptedMessageAtCurrentStage()) {
          ...
          mapOfMessageValidMaps.get(partitionId).put(indexWithinTheBucket, isValidMessage);
        }
  ...
  public void enqueueLoggingAuditEvents(KafkaWritingTaskResult result,
    ...
    for (Map.Entry<Integer, LoggingAuditHeaders> entry : headersMap.entrySet()) {
        ...
        boolean messageValid = messageValidMap.get(listIndex);
