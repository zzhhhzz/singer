  ...
  Map<Integer, List<ProducerRecord<byte[], byte[]>>> messageCollation(
    ...
    try {
      ...
      for (int i = 0; i < validPartitions.size(); i++) {
        buckets.put(validPartitions.get(i).partition(), new ArrayList<>());
        mapOfHeadersMaps.put(validPartitions.get(i).partition() ,new HashMap<Integer, LoggingAuditHeaders>());
      }
      ...
      for (LogMessage msg : logMessages) {
        ...
        if (msg.getLoggingAuditHeaders() != null) {
          ...
          mapOfHeadersMaps.get(partitionId).put( buckets.get(partitionId).size(), msg.getLoggingAuditHeaders());
        }
    ...
    try {
      ...
      for (Future<KafkaWritingTaskResult> f : resultSet) {
        ...
        if (!result.success) {
          ...
          if (this.enableLoggingAudit && SingerSettings.getLoggingAuditClient() != null) {
