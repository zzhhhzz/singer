...
public class KafkaUtils {
  ...
  public static KafkaProducer<byte[], byte[]> createKafkaProducer(KafkaProducerConfig config, String name) {
    String brokerList = Joiner.on(',').join(config.getBrokerLists());
    Properties properties = new Properties();
    properties.put(
        ProducerConfig.CLIENT_ID_CONFIG, name + "_" + UUID.randomUUID());
  ...
  public static ZkUtils getZkUtils(String zkUrl) {
    if (!zkUtilsMap.containsKey(zkUrl)) {
      Tuple2<ZkClient, ZkConnection>
          zkClientAndConnection = ZkUtils.createZkClientAndConnection(zkUrl, 300, 3000000);
