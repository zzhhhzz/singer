...
public class ConfigUtils {
  ...
  public static LoggingAuditClientConfig createLoggingAuditClientConfigFromKVs(Map<String, String> properties) throws ConfigurationException{
      ...
      if (!properties.containsKey(LoggingAuditClientConfigDef.STAGE)){
        ...
        loggingAuditClientConfig.setStage(LoggingAuditStage.valueOf(properties.get(LoggingAuditClientConfigDef.STAGE).toUpperCase()));
      }
      if (properties.containsKey(LoggingAuditClientConfigDef.DEFAULT_ENABLE_AUDIT_FOR_ALL_TOPICS)){
        loggingAuditClientConfig.setEnableAuditForAllTopicsByDefault(Boolean.valueOf(properties.get(LoggingAuditClientConfigDef.DEFAULT_ENABLE_AUDIT_FOR_ALL_TOPICS)));
      }

      if (properties.containsKey(LoggingAuditClientConfigDef.QUEUE_SIZE)){
        loggingAuditClientConfig.setQueueSize(Integer.valueOf(properties.get(LoggingAuditClientConfigDef.QUEUE_SIZE)));
      }

      if (properties.containsKey(LoggingAuditClientConfigDef.ENQUEUE_WAIT_IN_MILLISECONDS)){
        loggingAuditClientConfig.setEnqueueWaitInMilliseconds(Integer.valueOf(properties.get(LoggingAuditClientConfigDef.ENQUEUE_WAIT_IN_MILLISECONDS)));
      }
      ...
      Set<String> brokerSet = Sets.newHashSet(properties.get(LoggingAuditClientConfigDef.BOOTSTRAP_SERVERS).split(","));
      String acks = properties.containsKey(LoggingAuditClientConfigDef.ACKS) ? properties.get(LoggingAuditClientConfigDef.ACKS) : LoggingAuditClientConfigDef.DEFAULT_ACKS;
      acks = acks.toLowerCase();
      KafkaProducerConfig kafkaProducerConfig = new KafkaProducerConfig("", Lists.newArrayList(brokerSet), acks);
  ...
  public static LoggingAuditClientConfig parseFileBasedLoggingAuditClientConfig(String loggingAuditClientConfigFile) throws ConfigurationException {
  ...
  public static LoggingAuditClientConfig parseLoggingAuditClientConfig(AbstractConfiguration conf) throws ConfigurationException{
    LoggingAuditClientConfig loggingAuditClientConfig = parseCommonConfig(conf);
    loggingAuditClientConfig.setAuditConfigs(parseAllAuditConfigs(new SubsetConfiguration(conf, LoggingAuditClientConfigDef.AUDITED_TOPICS_PREFIX)));
    loggingAuditClientConfig.setSenderConfig(parseSenderConfig(new SubsetConfiguration(conf, LoggingAuditClientConfigDef.SENDER_PREFIX)));
    return loggingAuditClientConfig;
  }



  public static LoggingAuditClientConfig parseCommonConfig(AbstractConfiguration conf) throws ConfigurationException{
    LoggingAuditClientConfig loggingAuditClientConfig = new LoggingAuditClientConfig();
    try {
      loggingAuditClientConfig.setStage(LoggingAuditStage.valueOf(conf.getString(LoggingAuditClientConfigDef.STAGE).toUpperCase()));
    } catch (Exception e){
      throw new ConfigurationException("STAGE is not properly set!");
    }

    if (conf.containsKey(LoggingAuditClientConfigDef.DEFAULT_ENABLE_AUDIT_FOR_ALL_TOPICS)){
      loggingAuditClientConfig.setEnableAuditForAllTopicsByDefault(conf.getBoolean(LoggingAuditClientConfigDef.DEFAULT_ENABLE_AUDIT_FOR_ALL_TOPICS));
    }
    ...
    if (conf.containsKey(LoggingAuditClientConfigDef.ENQUEUE_WAIT_IN_MILLISECONDS)){
      loggingAuditClientConfig.setEnqueueWaitInMilliseconds(conf.getInt(LoggingAuditClientConfigDef.ENQUEUE_WAIT_IN_MILLISECONDS));
    }
    return loggingAuditClientConfig;
  }
  ...
  public static AuditConfig parseAuditConfig(AbstractConfiguration conf) throws ConfigurationException {
      ...
      try {
        ...
        if (conf.containsKey(LoggingAuditClientConfigDef.START_AT_CURR_STAGE)) {
          topicAuditConfig.setStartAtCurrStage(
              conf.getBoolean(LoggingAuditClientConfigDef.START_AT_CURR_STAGE));
        }
        if (conf.containsKey(LoggingAuditClientConfigDef.STOP_AT_CURR_STAGE)) {
          topicAuditConfig
              .setStopAtCurrStage(conf.getBoolean(LoggingAuditClientConfigDef.STOP_AT_CURR_STAGE));
        }
  ...
  public static AuditConfig createAuditConfigFromKVs(Map<String, String> properties) throws ConfigurationException{
    AuditConfig topicAuditConfig = new AuditConfig();
    try {
      ...
      if (properties.containsKey(LoggingAuditClientConfigDef.START_AT_CURR_STAGE)) {
        topicAuditConfig.setStartAtCurrStage(
            Boolean.valueOf(properties.get(LoggingAuditClientConfigDef.START_AT_CURR_STAGE)));
      }
      if (properties.containsKey(LoggingAuditClientConfigDef.STOP_AT_CURR_STAGE)) {
        topicAuditConfig.setStopAtCurrStage(
            Boolean.valueOf(properties.get(LoggingAuditClientConfigDef.STOP_AT_CURR_STAGE)));
      }
      return topicAuditConfig;
    } catch (Exception e){
  ...
  public static LoggingAuditEventSenderConfig parseSenderConfig(AbstractConfiguration conf) throws ConfigurationException{
    try{
      LoggingAuditEventSenderConfig senderConfig = new LoggingAuditEventSenderConfig();
      SenderType senderType = SenderType.valueOf(conf.getString(LoggingAuditClientConfigDef.SENDER_TYPE).toUpperCase());
      ...
      senderConfig.setKafkaSenderConfig(parseKafkaSenderConfig(new SubsetConfiguration(conf, LoggingAuditClientConfigDef.KAFKA_SENDER_PREFIX)));
      return senderConfig;
    } catch (Exception e) {
      throw new ConfigurationException("LoggingAuditEventSenderConfig can't be properly parsed due to " + e.getMessage());
    }
  }


  public static KafkaSenderConfig parseKafkaSenderConfig(AbstractConfiguration conf) throws ConfigurationException{
    KafkaSenderConfig kafkaSenderConfig = new KafkaSenderConfig();
    ...
    kafkaSenderConfig.setKafkaProducerConfig(parseProducerConfig(new SubsetConfiguration(conf, LoggingAuditClientConfigDef.KAFKA_PRODUCER_CONFIG_PREFIX)));
  ...
  public static KafkaProducerConfig parseProducerConfig(AbstractConfiguration producerConfiguration) throws ConfigurationException {
    ...
    Set<String> brokerSet = Sets.newHashSet(producerConfiguration.getStringArray(LoggingAuditClientConfigDef.BOOTSTRAP_SERVERS));
    String acks = producerConfiguration.containsKey(LoggingAuditClientConfigDef.ACKS) ? producerConfiguration.getString(LoggingAuditClientConfigDef.ACKS) : LoggingAuditClientConfigDef.DEFAULT_ACKS;
    acks = acks.toLowerCase();
    KafkaProducerConfig kafkaProducerConfig = new KafkaProducerConfig("", Lists.newArrayList(brokerSet), acks);

    if (producerConfiguration.containsKey(LoggingAuditClientConfigDef.COMPRESSION_TYPE)) {
      String compressionType = producerConfiguration.getString(LoggingAuditClientConfigDef.COMPRESSION_TYPE);
    ...
    if (producerConfiguration.containsKey(LoggingAuditClientConfigDef.SSL_ENABLED_CONFIG)) {
      boolean enabled = producerConfiguration.getBoolean(LoggingAuditClientConfigDef.SSL_ENABLED_CONFIG);
      ...
      if (enabled) {
        ...
        Iterator<String> sslKeysIterator = producerConfiguration.getKeys(LoggingAuditClientConfigDef.SECURE_KAFKA_PRODUCER_CONFIG_PREFIX);
    ...
    if (producerConfiguration.containsKey(LoggingAuditClientConfigDef.TRANSACTION_TIMEOUT_MS_CONFIG)) {
      int timeoutMs = producerConfiguration.getInt(LoggingAuditClientConfigDef.TRANSACTION_TIMEOUT_MS_CONFIG);
